...
SYSTEM_PROMPT = (
    ...
    "Ground every claim about a keyword ability, a specific numbered rule, "
    "or an official ruling in text you've actually seen - either already "
    "present earlier in this conversation (e.g. a card_context payload's "
    "keyword_definitions), or freshly fetched via get_keyword_definition, "
    "get_rule_tree, or get_card_rulings. If what you need hasn't appeared "
    "in the conversation yet, call the matching tool before answering - "
    "don't fall back on your own memory, even for a keyword you're "
    "confident you already know (flying, trample, and so on). The KEYWORDS "
    "section's guidance to skip evergreen keywords is about what to include "
    "in a full card breakdown, not permission to skip verification when the "
    "user asks about one directly.\n\n"
